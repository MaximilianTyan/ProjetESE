...
MFRC522::MFRC522(SPI_HandleTypeDef *hspi, GPIO_TypeDef *cs_port, uint32_t cs_pin, GPIO_TypeDef *reset_port,
                 uint32_t reset_pin) {
    this->hspi = hspi;
...
//  wait_ms(50);
    ...
    PCD_WriteRegister(TPrescalerReg,
                      0xA9); // TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25us.
...
uint8_t MFRC522::PICC_HaltA() {
    ...
    if (result == STATUS_OK) {
        ...
        if (result == STATUS_TIMEOUT) {
            result = STATUS_OK;
        } else if (result == STATUS_OK) { // That is ironically NOT ok in this case ;-)
            result = STATUS_ERROR;
        }
    }
...
void MFRC522::PCD_StopCrypto1() {
    ...
    PCD_ClrRegisterBits(Status2Reg,
